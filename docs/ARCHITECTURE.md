...
- La branche locale CAMP-2 exige une version de schéma au moins égale à `27`, portée par `0027` après `0025` et `0026`. Ces trois migrations Campagnes ne sont appliquées dans aucun environnement par ce travail local.
- Les migrations `0012` à `0027` restent manuelles. Le workflow vérifie le marqueur avant toute mutation Azure, puis `/api/health` et `/api/ready` après déploiement.
