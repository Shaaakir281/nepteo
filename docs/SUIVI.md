...
- **Readiness de schéma active** : `0016` introduit le marqueur privé après avoir prouvé les invariants critiques de `0012` à `0015`. `0019` contrôle les frontières finales et `0020` certifie la preuve terrain/cohorte ; `/api/ready` échoue fermée si Supabase ou la version requise sont indisponibles, et le workflow exige la version 20 avant toute mutation Azure.
- **Base et application alignées en production** : les migrations `0012` à `0020` ont été appliquées manuellement le 2026-07-30 et `app_schema_version = 20` est enregistré depuis `2026-07-30T06:02:14Z`. Azure sert l'image `nepteoacr27de3b.azurecr.io/nepteo:5d03f109e9d06c456781d72c2c0b5ab13eca1a4c`, révision `nepteo-prod--0000006`, avec 100 % du trafic.
- **Qualité de release** : les CI des PR #7 à #9 et celles de `main` ont validé tests, lint, typecheck et build. Le workflow final `30540910670` a validé le préflight schéma 20, construit l'image immuable, mis à jour Azure puis contrôlé liveness et readiness. La recette administrateur en lecture ne remonte aucune erreur console.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-07-30, la base est à la version 20 et Azure sert la révision `nepteo-prod--0000006`, image `5d03f10`, avec 100 % du trafic. `/`, `/api/health` et `/api/ready` répondent HTTP 200.

## Prochaines étapes (dans l'ordre)

1. **Achever la recette contrôlée du marqueur démo** : le code reconnaît désormais le connecteur démo orphelin et explique son blocage dans les cartes OAuth. Autoriser explicitement son retrait ciblé, vérifier qu'aucune donnée réelle n'est supprimée, puis re-recetter Google Sheets et Notion.
...
**Reste** : obtenir une autorisation explicite avant de retirer le marqueur démo orphelin de production, puis re-recetter les cartes et callbacks. Jouer le smoke RLS multi-rôles/tenants sur organisations `E2E_RLS_*`, les callbacks/synchronisations OAuth et les parcours C8/R1A/R1B/R2 avec mutation sur fixtures dédiées. C7 reste fermé et aucun envoi externe n'est activé.
...
**P1 connecteurs** : la production contient un unique connecteur `demo:connected` orphelin, sans prospect, campagne, revenu ni sauvegarde démo. Il provient du contrat de nettoyage de l'ancienne révision et bloque les cartes réelles ; la nouvelle action de retrait ne le reconnaît pas et ferait un no-op. Le correctif reconnaît désormais la combinaison réservée `provider=demo` + `config.demo=true`. Le retrait ne cible que les lignes `source=demo`, refuse tout connecteur portant encore une fiche non-démo ou tout état de marqueurs désaligné, puis revérifie l'absence de marqueur avant de journaliser le succès.
