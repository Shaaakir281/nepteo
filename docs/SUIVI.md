...
> **Références de recette** : `docs/demo/GUIDE-TEST.md` pour le parcours, `docs/tests/SCORECARD-COMMANDITAIRE.md` pour mesurer la valeur, `docs/TESTS.md` pour les connecteurs et `docs/tests/SMOKE-AUTH-RLS.md` pour l'isolation Supabase. Au 2026-08-14, Azure sert à 100 % la révision corrective `nepteo-prod--0000038`, sur l'image du merge `6fcbf95878444edeaf0e2a333e129ab67842a44e`. La production et le staging partagé sont réconciliés sur la chaîne canonique et attestés au schéma 32. Le smoke BUDGET-RESULTS avec un rôle Lecture dans une organisation dédiée ne remplace pas le smoke RLS multi-rôles/tenants complet, qui reste réservé à des organisations de recette séparées.

## État actuel (2026-08-14)

**CAMP-0, CAMP-1, CAMP-2, CONN-0, CONN-1, META-READ, META-METRICS, le parcours pilote Meta, BUDGET-RESULTS, le Studio de visuels campagne-first, le scénario en un clic et le durcissement sécurité sont fusionnés dans `main` et servis par la révision courante `nepteo-prod--0000038`.** META-METRICS lit uniquement via `ads_read` et applique ses photographies atomiques dans le schéma de production 32 ; aucune mutation Ads n'est ouverte. BUDGET-RESULTS est déployé et recetté techniquement sur l'état « Données absentes » avec un rôle Lecture dans une organisation dédiée sans connecteur. Aucun état runtime `ready` ni aucune donnée Meta non vide n'a été recetté : G1 et G2 restent ouverts, et déploiement, recette technique et recette sur données Meta réelles non vides restent des preuves séparées.
...
META-METRICS est déployé et le projet Supabase lié est attesté au schéma `32`
...
**Mise à jour du 10 août — simplifications d'authentification fusionnées et conservées.** La PR #31 a ajouté une déconnexion explicite dans l'en-tête mobile et un contrôle partagé « Afficher/Masquer le mot de passe » sur la connexion et la création de compte. Livrées initialement dans `nepteo-prod--0000023`, ces simplifications restent incluses dans la révision courante `0000030`. Ce micro-lot n'ajoutait aucune migration Supabase et ne modifiait aucun connecteur.

**Email de confirmation — redirection applicative corrigée, recette positive encore ouverte.** La Site URL, l'unique Redirect URL `/auth/confirm`, le modèle français et `{{ .ConfirmationURL }}` étaient correctement configurés. Un nouvel email français a pourtant reproduit `0.0.0.0` : la révision `0000023` reconstruisait les sorties succès et erreur de `/auth/confirm` depuis `request.url`, dont l'origine derrière Azure était l'adresse interne du conteneur. La PR #33 impose désormais `APP_URL` comme origine publique, échoue fermée si elle est invalide et conserve les flux `code` et `token_hash`. Les quatre smokes négatifs du callback sont verts sur `0000024` ; le clic unique d'un email neuf émis après le hotfix, avec jeton non consommé, puis la confirmation de session et l'arrivée dans l'application restent à recetter.
...
- **Actions Campagnes et ratios du Studio — déployés et recettés dans `0000028`** : « Nouvelle campagne » est remonté dans l'en-tête et reste disponible pour tout éditeur avant remplissage de la fiche, sans dépendre des métriques ni de l'onglet actif. « Créer un visuel » conserve l'accès libre au Studio et remplace le libellé trop étroit « Créer une story ». Les aperçus Story, carré et paysage conservent désormais respectivement leurs ratios 9:16, 1:1 et 3:2 sur mobile comme sur ordinateur ; génération, téléchargement et garde-fous serveur ne changent pas. La PR #39, la CI `31490079307`, le déploiement `31490242480` et la recette authentifiée sans génération payante sont verts.
- **Hiérarchie de l’onglet Décision — déployée et recettée dans `0000029`** : le résumé comptable ne sert plus de décision principale. Sans recommandation prioritaire, le héros annonce « Aucune décision prioritaire avec les données disponibles », puis sépare un éventuel point de vigilance déterministe — baisse conjointe des conversions et hausse du coût par conversion, ou baisse conjointe du revenu et du ROAS — des trois KPI et des preuves repliées. Les blocages documentés et les campagnes en perte restent les deux familles de décisions prioritaires existantes ; aucun prompt, appel IA, seuil d’exécution ou effet Ads n’est ajouté. La PR #41, sa CI `31495416696`, la CI `main` `31495546056`, le déploiement `31495719677` et la recette authentifiée sont verts.
...
- **Schéma Campagnes + Créatif + META-METRICS — Supabase à 32, code META-METRICS déployé** : la chaîne canonique `0022` à `0032` est présente dans le projet lié, qui expose `app_schema_version = 32`, `ad_metric_sync_runs`, `ad_campaigns`, `ad_metric_results` et `meta_ads_pilot_access_requests`. L'historique CLI 0001–0029, auparavant vide car les migrations avaient été appliquées par l'éditeur SQL, a été aligné après attestation du marqueur 29 et des trois tables META-METRICS. La production a été contrôlée au schéma 30 avant l'application de `0031/0032` ; le dry-run final ne relève aucun reliquat. Le smoke META-METRICS atomique/inter-tenant reste distinct.
...
- **BUDGET-RESULTS — fusionné, déployé et recetté techniquement sur l'état absent, non recetté sur données Meta non vides** : le cockpit en lecture seule est dédié aux lignes `provider_reported` de `ad_metrics`, `ad_campaigns` et `ad_metric_results`, sans les injecter dans l'ancien parcours financier CAMP-2. Il affiche dépense, résultats Meta typés, coût par résultat, fenêtres et tendances 7/30 jours, devise, fuseau, attribution, fraîcheur, provenance et qualité. Faute de rapprochement persistant explicite dans le schéma, le prévu affiche toujours « Budget prévu non rapproché » en runtime ; faute de champ fournisseur, son budget reste indisponible. Les sept états `ready`, `empty`, `missing`, `stale`, `partial`, `incompatible` et `error` sont couverts sans transformer l'absence en zéro ; une photographie complète de plus de 48 heures est `stale` avant toute classification `empty` ou `missing`. La mini-recette authentifiée avec un rôle Lecture dans une organisation dédiée sans connecteur a prouvé « Données absentes », les métadonnées indisponibles, la qualité « Absent », l'absence de campagne et l'absence de toute action, puis la fixture a été supprimée. Elle ne prouve ni l'état runtime `ready` ni des calculs sur données Meta non vides. Le lot n'ajoute ni recommandation, ni pause, ni mutation Meta, et n'affiche aucun revenu, CAC ou ROAS réel sans preuve aval. Les 688 tests, les 17 tests ciblés, typecheck, lint et le build de 29 pages/routes sont verts ; G1 et G2 restent ouverts.
...
- **Simplifications auth mobile — livrées dans `0000023`, incluses dans `0000024`** : la déconnexion est désormais visible dans l'en-tête sous le breakpoint desktop, sans ajouter une sixième destination à la barre mobile. Connexion et inscription partagent un champ mot de passe qui conserve la valeur lors du basculement entre texte visible et masqué, avec libellé accessible. Le lot initial passait 574/574 tests ; la release courante en passe 575/575. Le contrôle production confirme Afficher/Masquer sur `/login` et `/signup`. Le clic Déconnexion puis Retour depuis une session mobile authentifiée reste à recetter.
...
- **Readiness de schéma active** : `0016` introduit le marqueur privé. Après inspection de l'historique divergent, le staging puis la production ont appliqué la chaîne canonique jusqu'à `0032` et sont vérifiés à readiness 32. La production a été contrôlée au schéma 30 avant l'application de `0031/0032` ; le dry-run final est à jour.
- **Release applicative courante attestée** : la feature PR #48 est fusionnée au merge `0dbfb66d1d03298cf2050adcd90afa3b4cac11f0` après les CI de PR `31797939436` et de `main` `31798063966`, puis déployée par `31798410404` dans `nepteo-prod--0000037`. Le correctif de priorité de fraîcheur est fusionné via la PR #49, head `8188343294e5c4b13f82996141aaa29ddd1a74b8`, au merge `6fcbf95878444edeaf0e2a333e129ab67842a44e`, après les CI de PR `31799996913` et de `main` `31800104543`. Le déploiement `31800195369` est vert ; l'image du merge est servie par `nepteo-prod--0000038`, latest et ready, `Healthy`/`Provisioned`/`RunningAtMaxScale`, avec une réplique et 100 % du trafic.
- **Qualité de release et recettes ciblées** : 688/688 tests, les 17 tests BUDGET-RESULTS ciblés, typecheck, lint et le build de 29 routes sont verts. `/api/health` et `/api/ready` répondent 200 sur le domaine public et le FQDN Azure ; `/login` répond 200 et `/campagnes` sans session répond 307 vers `/login`. La mini-recette authentifiée avec un rôle Lecture dans une organisation dédiée sans connecteur a ouvert l'onglet « Budget et résultats » et vérifié « Données absentes »/« Absent », dernière collecte, devise, fuseau, attribution, fraîcheur et provenance « Indisponible », qualité « Absent », aucune campagne et aucun bouton ou lien dans la région. Aucun revenu, CAC ou ROAS n'était affiché ; aucun appel Meta, mutation, recommandation ou pause n'a été effectué, et aucun secret n'a été ajouté ou exposé. Après déconnexion, organisation, membership et utilisateur ont été supprimés et vérifiés absents. Cette recette ne prouve aucun état runtime `ready` ni aucun calcul sur données Meta non vides. `/login` sert bien CSP — avec les deux origines Google Fonts nécessaires — HSTS, `DENY`, `nosniff`, `strict-origin-when-cross-origin`, COOP et la restriction des permissions. La recette mobile authentifiée du scénario en un clic reste valide. Sur une organisation vide à 390 × 844, « + Nouvelle campagne » et « Créer un visuel » sont visibles avant toute fiche ou métrique et le document ne déborde pas. Sans génération payante, le cadre Story mesure 268 × 476,44 px sur mobile puis 332 × 590,22 px sur ordinateur ; carré et paysage mesurent 590 × 590 px et 885 × 590 px. La recette authentifiée de Décision confirme à 390 × 844 et 1280 × 720 le titre neutre, la vigilance `+17,4 % / −21,1 % / +48,7 %`, les trois KPI, les preuves sourcées et l'action « Rechercher des actions à valider », sans débordement, erreur navigateur ni appel fournisseur. La console est vide et les fixtures synthétiques ont été entièrement supprimées, avec utilisateur, membership, organisation et métriques vérifiés absents. OAuth Meta réel est désormais prouvé pour un compte vide ; Google Sheets, Notion et l'échantillon Meta non vide restent ouverts, ainsi que le clic positif d'un email neuf et le clic Déconnexion puis Retour depuis une session mobile authentifiée. Le gate `reset → reseed → préparation → exécution` reste ouvert.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-08-14, la base est à la version 32 et la révision déployée est `nepteo-prod--0000038`, sur l'image `nepteoacr27de3b.azurecr.io/nepteo:6fcbf95878444edeaf0e2a333e129ab67842a44e`. Elle est latest et ready, `Healthy`/`Provisioned`/`RunningAtMaxScale`, avec une réplique et 100 % du trafic. Les contrôles `/api/health` et `/api/ready` répondent 200 sur le domaine public et le FQDN Azure ; `/login` répond 200 et la garde de `/campagnes` répond 307 vers `/login` sans session.
...
production portait `0029_meta_metrics.sql` puis `0030_meta_ads_pilot_access.sql`
et a été contrôlée au schéma 30 avant la promotion du lot. Après les CI vertes,
elle a appliqué `0031_connector_foundation.sql` puis
`0032_connector_conflict_http.sql` et expose désormais readiness 32 avec un
dry-run à jour. Le staging partagé avait porté sous les numéros `0029/0030`
...
2. **META-METRICS — migrations `0029/0030`, production au schéma 32, G1 encore
   ouvert** : ne rejouer ni `0029` ni `0030` en production. La lecture vide ne remplace pas le tuple
   compte/campagne/date/dépense/type de résultat ; rapprocher un échantillon Meta
   non vide autorisé reste la preuve manquante.
3. **BUDGET-RESULTS — déployé et recetté techniquement sur l'état absent pendant
   G1** : dépense, résultats Meta, coût par résultat, tendances 7/30 jours,
   fraîcheur et qualité sont prêts à lire des données compatibles. En runtime,
   le prévu reste « Budget prévu non rapproché » faute de mapping persistant et
   le budget fournisseur reste indisponible faute de champ source. Les absences,
   incompatibilités et erreurs restent explicites ; CAC, revenu et ROAS restent
   indisponibles si leur source aval n'est pas vérifiée. Aucun état `ready` ni
   aucun calcul sur données Meta non vides n'a été recetté ; G2 reste ouvert.
...
### 2026-08-14 — Codex — **BUDGET-RESULTS déployé, correctif de fraîcheur actif, G1 et G2 ouverts**

- **Publication** : la feature PR #48 est fusionnée au merge `0dbfb66d1d03298cf2050adcd90afa3b4cac11f0` après les CI de PR `31797939436` et de `main` `31798063966`, toutes deux vertes. Le premier déploiement `31798410404` a activé `nepteo-prod--0000037`. Le correctif qui donne priorité à `stale` au-delà de 48 heures, y compris avant `empty` ou `missing`, est porté par la PR #49, head `8188343294e5c4b13f82996141aaa29ddd1a74b8`, merge `6fcbf95878444edeaf0e2a333e129ab67842a44e`, avec les CI de PR `31799996913` et de `main` `31800104543` vertes.
- **Migrations** : le staging avait déjà été réconcilié sur la chaîne canonique `0029_meta_metrics` → `0030_meta_ads_pilot_access` → `0031_connector_foundation` → `0032_connector_conflict_http` et readiness 32. La production a été inspectée et contrôlée au schéma 30 avant d'appliquer `0031/0032` ; elle expose maintenant `app_schema_version = 32` et son dry-run est à jour. Aucun historique n'a été écrasé et aucun `DROP` n'a été exécuté.
- **Déploiement correctif** : le workflow `31800195369` est vert. La révision finale `nepteo-prod--0000038` sert l'image `nepteoacr27de3b.azurecr.io/nepteo:6fcbf95878444edeaf0e2a333e129ab67842a44e` ; elle est latest et ready, `Healthy`/`Provisioned`/`RunningAtMaxScale`, avec une réplique et 100 % du trafic. `/api/health` et `/api/ready` répondent 200 sur le domaine public et le FQDN Azure, `/login` répond 200 et `/campagnes` sans session répond 307 vers `/login`.
- **Validations** : la suite locale finale passe 688/688 tests, les tests BUDGET-RESULTS ciblés 17/17, puis typecheck, lint et le build de 29 pages/routes. La revue du diff confirme l'absence d'appel Meta, d'`ads_management`, d'endpoint d'écriture, de recommandation, de pause, de mutation, de revenu, de CAC ou de ROAS non vérifié, ainsi que l'absence de secret ajouté ou exposé.
- **Mini-recette authentifiée sans appel Meta** : un rôle Lecture dans une organisation dédiée sans connecteur a ouvert l'onglet « Budget et résultats ». La région affiche « Données absentes »/« Absent » ; dernière collecte, devise, fuseau, attribution, fraîcheur et provenance sont « Indisponible », la qualité est « Absent », aucune campagne et aucun bouton ou lien n'apparaissent. Après déconnexion, organisation, membership et utilisateur ont été supprimés et vérifiés absents.
- **Vérité produit** : cette recette prouve le déploiement, la garde d'authentification et l'état absent, jamais un état runtime `ready` ni un calcul sur données Meta non vides. Faute de mapping persistant, le runtime affiche toujours « Budget prévu non rapproché » ; faute de champ source, le budget fournisseur reste indisponible. Aucun appel Meta n'a été joué. G1 et G2 restent ouverts ; `META-RECOMMEND` et `META-PAUSE` restent fermés.
...
- **Frontière** : la maquette commerciale ne certifie aucune capacité. Ce lot ne crée ni recommandation, ni pause, ni endpoint ou appel de mutation Meta, n'ajoute et n'expose aucun secret et ne joue aucun appel Meta pendant le développement ou les smokes techniques.
- **Implémentation et recette locale** : domaine pur fail-closed, lecture serveur tenant+compte Meta courant, onglet « Budget et résultats » et sept états sont en place. Les 16 tests du domaine, 8 tests d'intégration statique, 5 tests de réconciliation et les contrôles readiness passent ; la suite complète passe **687/687**, puis `npm run typecheck`, `npm run lint`, `npm run build` (29 pages/routes) et `git diff --check` sont verts. La revue statique ne trouve aucun appel Meta, `ads_management`, endpoint d'écriture, recommandation, pause, revenu, CAC ou ROAS dans le nouveau parcours, ni aucun secret ajouté ou exposé.
...
- Le parcours temporaire Meta est porté par la PR #46 derrière `0030` : demande d'accès par organisation/utilisateur, e-mail Facebook normalisé, lien de profil facultatif, états `requested → ready → connected`, lecture RLS du seul demandeur et RPC service-role pour les transitions.
- Une demande en attente ne propose plus de relancer OAuth. L'ajout du rôle testeur et le message au testeur restent explicitement manuels ; seul l'état `ready` déverrouille « Finaliser la connexion Meta ». Un utilisateur déjà ajouté comme testeur peut emprunter directement le parcours OAuth.
- Aucun mot de passe, jeton, App ID ou secret n'est demandé ; aucun appel Meta ni envoi n'a été effectué. Les commits `debb3f2` et `454ac7a`, la PR #46, ses CI et le déploiement protégé `31783151837` sont verts.
- `0030` a été appliquée sur le projet production après dry-run isolé : `app_schema_version = 30`, lecture service-role de la table pilote en HTTP 200, accès anonyme refusé en 401 et aucun reliquat de migration. Le staging partagé portait déjà une migration `0030` divergente d'un autre chantier et n'a donc pas été muté. G1 META-METRICS reste ouvert pour le smoke atomique/JWT et l'échantillon non vide.
...
**Dépendances** : Next.js et `eslint-config-next` passent à 16.2.11. `sharp` est épinglé à 0.35.3 et les résolutions transitives `postcss`/`nanoid` sont durcies ; `npm audit --omit=dev` comme l'audit complet ne remontent plus aucune vulnérabilité connue.

**OAuth et exposition HTTP** : un module commun signe l'état OAuth par HMAC-SHA256 avec séparation de domaine, le lie au fournisseur, à l'utilisateur et à l'organisation, le compare en temps constant et l'associe à un cookie navigateur court, `HttpOnly`, `SameSite=Lax` et `Secure` en production. Les autorisations et callbacks Google Sheets, Notion et Meta utilisent l'origine publique validée `APP_URL` pour les redirections applicatives et les `redirect_uri`, jamais l'origine interne du proxy. Les réponses ajoutent globalement CSP, HSTS, `X-Frame-Options: DENY`, `nosniff`, une politique de référent restrictive, COOP et une `Permissions-Policy` minimale.

**Contrôles et production** : les 642 tests, le lint, le typecheck avec incrémentalité désactivée et le build Next.js 16.2.11 de 29 routes passent sur le dernier `origin/main` ; l'audit npm complet retourne zéro vulnérabilité. Le commit `c105d2b` a été publié par la PR #43, dont la CI `31587995982` est verte, puis fusionné au merge `77a34ebd05c6d9290115b821b5dc4eeaf42de949` ; la CI `main` `31588318580` est verte. Le workflow protégé `31588447865` a promu l'image immuable sur `nepteo-prod--0000030`, latest et ready, `Succeeded`/`Running`, à 100 % du trafic. Santé, readiness et login répondent 200 ; CSP, HSTS, anti-iframe, `nosniff`, référent, COOP et restrictions de permissions sont présents. Le lot n'ajoute aucune migration, permission `ads_management`, écriture Ads, Conversions API, appel OAuth réel ou secret.

**Meta Andromeda** : Andromeda est traité comme un moteur interne de récupération/sélection publicitaire de Meta et non comme une API générative directement pilotable. Une future écriture Meta devra faire produire et valider à Nepteo un objet campagne structuré, puis le transmettre via l'API Marketing sous autorisation séparée ; Meta/Advantage+ conservera l'optimisation de diffusion. L'envoi ultérieur de conversions CRM via Conversions API formera un lot distinct avec consentement, minimisation, déduplication et recette dédiée.

### 2026-08-11 — Codex — **Décision Campagnes clarifiée, déployée et recettée**

**Diagnostic** : le grand titre reprenait `presentDailySummary`, donc un inventaire de campagnes, dépenses, conversions et revenu, lorsque le moteur ne trouvait aucune recommandation. « Aucune priorité supplémentaire » signifiait seulement qu’aucun blocage documenté non résolu ni aucune campagne en perte non déjà traitée ne passait les règles déterministes ; le « Pourquoi ? » citait pourtant les seuils de projection CAMP-1, sans rapport avec cette conclusion. Aucun agent conversationnel ni prompt IA ne produisait ce texte.

**Correctif** : le héros distingue désormais la décision, la vigilance et les preuves. La conclusion neutre est « Aucune décision prioritaire avec les données disponibles ». Un encart ambre signale séparément une efficacité d’acquisition en baisse lorsque les conversions reculent tandis que le coût par conversion augmente, ou un rendement en baisse lorsque revenu et ROAS reculent. Les totaux et leur source restent dans « Pourquoi cette conclusion ? », qui décrit les vraies règles. Le bouton « Analyser » devient « Rechercher des actions à valider ». La recommandation prioritaire existante conserve son titre propre, qu’elle concerne un blocage ou l’examen d’une campagne en perte.

**Contrôles et production** : les 52 tests ciblés et la suite complète 635/635 sont verts ; lint, typecheck, `git diff --check` et le build Next.js 16.2.10 de 29 routes terminent avec exit 0. Le commit `06e843c` a été publié par la PR #41, dont la CI `31495416696` est verte, puis fusionné au merge `71319e09f674bf90adc445eba288079decab1882` ; la CI `main` `31495546056` est verte. Le workflow protégé `31495719677` a promu l’image immuable sur `nepteo-prod--0000029`, latest et ready, `Succeeded`/`Running`, à 100 % du trafic ; santé, readiness et login répondent 200. La recette authentifiée de production reproduit les chiffres du retour à 390 × 844 et 1280 × 720 : titre, vigilance `+17,4 % / −21,1 % / +48,7 %`, trois KPI, explication sourcée et action sont cohérents, sans débordement horizontal, erreur navigateur ni appel IA ou fournisseur. L’acteur, le membership, l’organisation et les six métriques synthétiques ont ensuite été supprimés et vérifiés absents.

### 2026-08-11 — Codex — **Actions de création permanentes et aperçus aux bons ratios déployés et recettés**

**Deux gestes globaux** : la page Campagnes affiche désormais « + Nouvelle campagne » comme action principale et « Créer un visuel » comme action secondaire pour tout membre éditeur, avant même que la fiche entreprise ou les métriques soient renseignées et quel que soit l'onglet actif. La modale existante et son préremplissage depuis la mémoire sont réutilisés dans l'en-tête ; la copie auparavant conditionnelle dans le héros Décision est retirée. La navigation et le Studio emploient le terme générique « visuel », qui couvre Story 9:16, post carré 1:1 et paysage 3:2 sans modifier la recommandation issue du canal.

**Aperçu fidèle** : le cadre Story n'associe plus une largeur pleine à une hauteur maximale indépendante. Les trois formats reçoivent une largeur maximale dérivée d'une hauteur de 590 px — 332 px pour 9:16, 590 px pour 1:1 et 885 px pour 3:2 — tout en restant fluides sur petit écran. L'image générée, les dimensions fournisseur `1008×1792`, le téléchargement canvas, les quotas, les rôles, l'isolation démo et l'absence de publication automatique sont inchangés.

**Contrôles et production** : les contrats ciblés passent 37/37 et la suite complète 632/632 ; lint, typecheck et build Next.js 16.2.10 de 29 routes sont verts. Le commit `9bda0b9` a été publié par la PR #39, dont la CI `31490079307` est verte, puis fusionné au merge `cbfc2194afc0df52b38a8f62515195dcdea94331`. Le workflow protégé `31490242480` a promu l'image immuable sur `nepteo-prod--0000028`, latest et ready, `Succeeded`/`Running`, à 100 % du trafic ; santé, readiness et login répondent 200. La recette authentifiée sans génération payante confirme à 390×844, sur une organisation vide, les deux actions visibles avant toute fiche ou métrique, un document sans débordement horizontal et un cadre Story mesuré à 268×476,44 px ; à 1280×720, Story mesure 332×590,22 px, carré 590×590 px et paysage 885×590 px. La console est vide. L'utilisateur, le membership et l'organisation synthétiques temporaires ont été supprimés et leur absence vérifiée.

### 2026-08-11 — Codex — **Scénario en un clic et identité progressive déployés**

**Diagnostic du triple geste** : le premier clic ne faisait que sélectionner une radio locale dans `OnboardingExample`, le deuxième soumettait `createOrganization` puis redirigeait vers le guide, le troisième redirigeait encore vers Connecteurs, où « Charger et analyser » exécutait enfin le seed protégé. Le parcours retenu conserve le choix initial « Voir Nepteo travailler », puis transforme chaque carte d'entreprise en intention explicite unique : choisir la carte crée l'espace vide et lance immédiatement ce scénario, sans chargement implicite à l'arrivée sur la page.

**Causalité visible et compacte** : après le clic, l'utilisateur reste sur place et voit « Nepteo analyse », huit cases en attente, puis l'identité se révéler rubrique par rubrique — activité, zone, offres, ton, philosophie, canaux, communication et objectifs. Le badge « Démonstration fictive · aucun site consulté » interdit toute confusion avec un vrai site ; aucun domaine fictif, appel web, recherche payante ou écriture fournisseur n'est introduit. Une fois 8/8 atteint, « Commencer la prise en main » ouvre naturellement le guide sur la fiche déjà remplie.

**Frontières conservées** : l'onboarding et le panneau Connecteurs utilisent désormais la même fonction de chargement. Le verrou distribué, le préflight fail-closed, les marqueurs de seed, les scopes prospects/campagnes et leur journal restent uniques ; la contrainte d'un seul membership ferme les courses entre doubles clics. Un compte déjà configuré ou toute donnée imprévue bloque le parcours. La voie entreprise réelle et l'ancien parcours Connecteurs restent compatibles.

**Contrôles et production** : travail parti du merge `3700dd71fcd7f377c02ab2ba828faf959da197cc` sur la branche isolée `codex/scenario-one-click-demo`. Les tests ciblés passent 33/33 et la suite complète 632/632 ; `npm run lint`, `npm run typecheck` et `npm run build` terminent avec exit 0. La PR #37 a passé la CI `31485168048`, puis a été fusionnée au merge `ec43a8fff525c08a5c33ad3491cb45fac73f4b78`. Le workflow protégé `31485378634` a vérifié la cible Azure et le schéma 28, puis promu l'image immuable correspondante sur `nepteo-prod--0000027`, latest et ready, `Succeeded`/`Running`, à 100 % du trafic. La recette authentifiée production à 390 × 844 montre les huit rubriques, un document de 390 px sans débordement horizontal, un viewport entier sans scroll supplémentaire et aucune erreur ou alerte console ; le CTA arrive bien sur `/prise-en-main?depart=example&scenario=artisan`. Le scénario a ensuite été retiré, l'utilisateur synthétique supprimé et son membership ramené à zéro ; seule la coquille d'organisation liée au journal append-only subsiste.
...
**Préparation de la suite** : les chemins sentinelles ont été corrigés et leurs longueurs remesurées sur le code après refonte. Les quatre prompts UX-6 à UX-9 ont été intégrés au guide avec une recette responsive sur le PC, sans référence trompeuse à un appareil mobile. Une première recherche limitée au worktree et aux pièces jointes avait conclu à tort que les quatre maquettes manquaient. Après indication de Fathi, `nepteo-validation-ux6.html`, `nepteo-listes-ux7.html`, `nepteo-brancher-ux8.html` et `nepteo-produire-ux9.html` ont été retrouvés dans `C:\dev\agent_marketing\docs\maquettes` puis importés localement. Le README distingue maintenant les cinq lots terminés des quatre specs disponibles pour la suite.

**Fins de ligne et frontière** : le checkout était propre avant cette passe ; l'alerte antérieure des 157 fichiers CRLF ne décrit donc pas son état courant. Le risque reste documenté parce que `.gitattributes` est absent et que Git système utilise `core.autocrlf=true`. Les quatre maquettes ont seulement reçu la fin de fichier LF attendue lors de leur import ; leur contenu est identique au checkout principal. Aucune renormalisation globale, aucun fichier applicatif, test, secret, état distant, push, PR, publication ou déploiement n'a été touché.
...
**Revue conjointe** : UX-1 ne modifie aucun fichier d'onboarding et UX-5 ne modifie aucun fichier Aujourd'hui. Le diff combiné ne touche aucune Server Action, garde de rôle/démo, migration, route, moteur d'analyse, moteur d'exécution ou moteur de priorité. Le choix onboarding reste compact et exclusif ; sélectionner un scénario ne le charge pas. Aujourd'hui partage bien `WALKTHROUGH_STORAGE_KEY` et `WALKTHROUGH_UPDATED_EVENT`, expose `x/5`, un seul bouton plein dans son héros et le libellé visible « Pourquoi ? » dont seule la réponse est repliée.

**Réserves constatées, non masquées** : le contrat serveur antérieur redirige encore la création d'un espace d'exemple vers `/prise-en-main?depart=example&scenario=…`, et non directement vers Aujourd'hui. Cet écran de prise en main, hors des deux lots intégrés, affiche encore le nombre de missions sur `WALKTHROUGH_MISSIONS.length`, soit `/11`, tandis qu'Aujourd'hui affiche `x/5`. Corriger ces deux points imposerait de modifier la Server Action d'onboarding et le lot UX-2 ; aucune extension de périmètre n'a été faite pendant cette intégration. La recette navigateur authentifiée reste également conditionnée à un environnement Supabase local configuré, sans copie de secret.

**Contrôles** : les dépendances exactes du checkout ont été matérialisées depuis le cache npm avec `npm ci --offline --ignore-scripts --no-audit --no-fund` (455 paquets, aucun accès réseau). `git diff --check` est vert avec le seul avertissement LF→CRLF sur ce journal ; `npm run lint` et `npm run typecheck` terminent avec exit 0 ; `npm test` passe 582/582, 0 échec ; `npm run build` termine avec exit 0, compile en 43 s, finit TypeScript en 15,3 s et génère 29 pages statiques avant de construire toutes les routes dynamiques. La recette navigateur authentifiée n'a pas été lancée : aucun `.env.local` ni variable `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` ou `SUPABASE_SERVICE_ROLE_KEY` n'est présent dans ce checkout ; aucun secret n'a été copié. Aucun push, PR, publication, déploiement, appel externe ni donnée distante n'a été créé ou modifié.
...
### 2026-08-10 — Codex — **UX-5 local : onboarding épuré à 38 mots**

**Fait** : l’écran de choix pose désormais la seule question « Par où voulez-vous commencer ? », puis présente deux voies exclusives en lignes illustrées, avec les durées « 3 min » et « 5 min ». Le sous-titre et les badges redondants ont disparu ; « Mode sûr » tient sur une ligne centrée de 11,5 px sans changer la promesse. L’écran suivant réduit les trois scénarios à un nom, un contexte et une icône métier réutilisée depuis `components/icons.tsx`. Le composant initial de 259 lignes a été découpé en composants courts, un par fichier.

**Décisions** : les textes et la hiérarchie reprennent la spec `nepteo-onboarding-ux5.html` au mot près pour le premier écran, soit 38 mots visibles. Les champs cachés, le formulaire réel et `app/onboarding/actions.ts` restent inchangés. Choisir un scénario ne fait que mettre à jour la sélection locale ; créer l’espace reste un geste séparé et le scénario n’est toujours chargé qu’après confirmation explicite dans le cockpit.

**Validation et reste** : le test ciblé passe 7/7 et la suite complète 577/577 ; `npm run lint`, `npm run typecheck`, `npm run build` (29 pages) et `git diff --check` sont verts. La tentative de recette à 390 × 844 a été arrêtée avant l’écran par l’absence des variables Supabase dans ce worktree : le rendu authentifié reste à contrôler sur un compte neuf dans un environnement local configuré. Aucun fichier du cockpit Aujourd’hui, aucune Server Action, migration, route, télémétrie, donnée distante, publication ou déploiement n’a été modifié ou déclenché.
...
**Promotion technique** : la PR [#33](https://github.com/Shaaakir281/nepteo/pull/33) a passé la CI `31372881341` avec 575/575 tests, lint, typecheck et build de 29 routes, puis a été fusionnée au SHA `6f54cee5cc7c594956515cea2a94204d5da2a5a2`. Le workflow protégé `31373032666` a vérifié le schéma 28, construit l'image immuable correspondante et activé `nepteo-prod--0000024` à 100 %, latest et ready, `Succeeded`/`Running`.

**Preuve production et reste** : les callbacks sans paramètre, `token_hash` avec `type=email`, `token_hash` avec `type=signup` et `code=invalid` répondent tous 307 vers `https://nepteo.bogasolution.com/login?...`, sans `0.0.0.0` ; health et readiness répondent 200. Cette preuve ferme le défaut de redirection applicative, pas le parcours positif : un email neuf émis après le hotfix, avec jeton non consommé, doit encore être cliqué une seule fois pour confirmer le compte, créer la session et arriver dans l'application. Le logout mobile authentifié reste également à jouer. Aucun schéma, connecteur ni contrat Campagnes n'a été modifié.

### 2026-08-10 — Codex — **Historique — PR #31 fusionnée et simplifications auth déployées**
...
**Production à cette date** : la révision était latest et ready, `Succeeded`/`Running`, Healthy/Provisioned/RunningAtMaxScale avec une réplique. `/` aboutissait à `/login` en 200 ; `/login`, `/signup`, `/api/health` et `/api/ready` répondaient 200, et health/readiness étaient aussi verts sur le FQDN Azure. Le navigateur de production confirmait la présence d'Afficher/Masquer sur connexion et inscription, le passage du champ de `password` à `text` et la conservation de la valeur saisie.

**Auth Supabase et reste au moment de cette release** : la Site URL, l'unique Redirect URL `/auth/confirm` et le modèle Confirm signup français étaient corrects. L'hypothèse alors retenue « ancien email uniquement » a été invalidée par un email français neuf ; la cause applicative et son hotfix sont consignés dans l'entrée PR #33 ci-dessus. Restent à jouer avec un compte de test dédié : clic positif d'un email neuf, puis Déconnexion et Retour depuis une session mobile authentifiée. Aucun schéma, connecteur, appel fournisseur, campagne ni donnée métier n'a été modifié par ce lot.
