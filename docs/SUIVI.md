...
- **Readiness de schéma active** : `0016` introduit le marqueur privé après avoir prouvé les invariants critiques de `0012` à `0015`. `0019` contrôle les frontières finales et `0020` certifie la preuve terrain/cohorte ; le lot local `0021` ajoute le remplacement CSV atomique et relève la version applicative requise à 21. `/api/ready` échoue fermée si Supabase ou la version requise sont indisponibles. La production reste volontairement au schéma 20 tant que `0021` n'a pas été appliquée.
- **Base et application alignées en production** : les migrations `0012` à `0020` ont été appliquées manuellement le 2026-07-30 et `app_schema_version = 20` est enregistré depuis `2026-07-30T06:02:14Z`. Azure sert l'image `nepteoacr27de3b.azurecr.io/nepteo:a2bbc34dcb97ab00951a3efa631c4f7c0a0428ca`, révision `nepteo-prod--0000007`, avec 100 % du trafic.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-07-30, la base est à la version 20 et Azure sert la révision `nepteo-prod--0000007`, image `a2bbc34`, avec 100 % du trafic. `/`, `/api/health` et `/api/ready` répondent HTTP 200.

## Prochaines étapes (dans l'ordre)

1. **Appliquer `0021`, puis recetter les deux voies sans mélange** : certifier les trois variantes du scénario Nepteo V2 avec leurs comptages/signaux et le cycle idempotent `reset → reseed`, puis vérifier qu'il faut retirer ce scénario avant d'importer les données autorisées d'un testeur. Le CSV V1 doit rester UTF-8, borné à 900 Ko/5 000 lignes, mappé sans ambiguïté, minimisé, atomique, verrouillé et journalisé.
...
**Validation locale** : **359/359 tests**, typecheck, lint, `git diff --check` et build Next.js de production (**24 pages/routes**) sont verts. Les revues indépendantes démo et CSV ne relèvent plus de P1/P2 ; le CSV reste déployable sous condition d'appliquer `0021` puis de jouer son smoke RPC. Ce lot n'est pas encore commité, la migration `0021` n'est pas appliquée, et rien n'est déployé ni recetté dans le navigateur ; aucune donnée distante n'a été modifiée pendant cette session.
