...
> **Références de recette** : `docs/demo/GUIDE-TEST.md` pour le parcours, `docs/tests/SCORECARD-COMMANDITAIRE.md` pour mesurer la valeur, `docs/TESTS.md` pour les connecteurs et `docs/tests/SMOKE-AUTH-RLS.md` pour l'isolation Supabase. Au 2026-07-31, `/`, `/api/health` et `/api/ready` ainsi que les contrôles Azure répondent HTTP 200 sur la révision `nepteo-prod--0000008`. L'organisation présentée aux commanditaires est un environnement de test qui suit une seule voie à la fois : scénario Nepteo V2 certifié ou données autorisées du testeur. Le smoke RLS multi-rôles/tenants se valide dans des organisations de recette séparées.

## État actuel (2026-07-31)

**Phase technique 3A et lot R2 déployés ; porte terrain de la phase 2 encore ouverte.** L'agent lit, détecte, propose et prépare des messages dans l'outbox. Le lanceur R2, lui, crée uniquement une proposition : il n'écrit pas dans l'outbox et n'envoie **rien à l'extérieur**. La [roadmap valeur — tests commanditaires](projets/roadmap-valeur-commanditaires.md) gouverne désormais l'ordre des améliorations.
...
- **Deux voies de test exclusives déployées ; RPC CSV recettées et surfaces contrôlées** : **A**, l'un des trois scénarios Nepteo V2 certifiés dans une organisation vide, identifiable, réinitialisable et seul autorisé à porter le libellé « données fictives » ; **B**, des données autorisées apportées par le testeur via connecteur ou CSV, présentées comme « environnement de test ». Le scénario doit être retiré avant tout import. Le préflight, les marqueurs namespacés et le verrou distribué empêchent le mélange ; une sauvegarde corrompue bloque tout seed. Sans fencing distribué, un verrou orphelin reste bloquant jusqu'à récupération manuelle vérifiée. Le smoke réel de `0021` est vert sur `E2E_RLS_CSV_OWN` / `E2E_RLS_CSV_OTHER` : refus inter-tenant `42501`, import, rollback tardif, rejeu idempotent et double retrait. Le cycle complet `load → analyse → reset → reseed` reste à jouer sur chacune des trois variantes.
...
- **Readiness de schéma active** : `0016` introduit le marqueur privé après avoir prouvé les invariants critiques de `0012` à `0015`. `0019` contrôle les frontières finales, `0020` certifie la preuve terrain/cohorte et `0021` ajoute le remplacement CSV atomique. `/api/ready` échoue fermée si Supabase ou la version requise sont indisponibles ; le nouveau lot exige 21.
- **Base et application alignées en production** : `app_schema_version = 21` est enregistré depuis `2026-07-30T17:40:26Z`. Azure sert à 100 % l'image immuable `nepteoacr27de3b.azurecr.io/nepteo:813d2e0f3d49f19ec4d2c5094fe1e5f95af281ae`, digest `sha256:73b9566dcdafe12d01b472fa02c7ed1108bf042f7154631ec9ed01fa9283eca9`, sur la révision `nepteo-prod--0000008`.
- **Qualité de release** : la PR [#13](https://github.com/Shaaakir281/nepteo/pull/13) et sa [CI `30609476514`](https://github.com/Shaaakir281/nepteo/actions/runs/30609476514) sont vertes ; la [CI `main` `30609579805`](https://github.com/Shaaakir281/nepteo/actions/runs/30609579805) est verte. Le build ACR `dd8` a produit l'image et le digest recettés. Les contrôles publics et Azure de `/`, `/api/health` et `/api/ready` répondent HTTP 200. Le contrôle administrateur de surface dans le navigateur intégré confirme les trois cartes de scénario, le libellé prudent « environnement de test », l'accès CSV conditionné au retrait du scénario et une console sans erreur.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-07-31, la base est à la version 21 et Azure sert à 100 % la révision `nepteo-prod--0000008`, image `813d2e0f`, digest `sha256:73b9566dcdafe12d01b472fa02c7ed1108bf042f7154631ec9ed01fa9283eca9`. `/`, `/api/health` et `/api/ready` répondent HTTP 200.

## Prochaines étapes (dans l'ordre)

1. **Terminer le smoke RLS multi-rôles** : compte neuf, rôles, lecture/écriture autorisées et refus inter-tenant sur des organisations `E2E_RLS_*` ; contrôler à nouveau l'absence d'envoi externe.
2. **Recetter les connecteurs réels avant pilote** : callbacks et synchronisations OAuth Google Sheets/Notion, dates C8 et conflit de statut multi-source. Pour un même contact, tout statut terminal/opposé doit bloquer la relance et deux statuts actifs contradictoires doivent suspendre la cible, de la proposition jusqu'à la préparation.
3. **Recetter R1A, R1B et R2 sur fixtures dédiées** : Top 5, seuils 30/45, sélection oldest-first, exclusion des vagues antérieures, verdicts/retouches, cohorte atomique, conservation de `action_kind` et scorecard locale non-démo/fail-closed.
4. **Promouvoir sur un pilote dédié et mesurer la preuve terrain** : reporter l'image `813d2e0f` et la révision `nepteo-prod--0000008` dans la scorecard, puis réunir 3 testeurs/30 recommandations pour le checkpoint qualitatif et 50 recommandations/deux semaines pour la décision produit, avec consolidation programme pseudonymisée hors application et sans agrégation inter-tenant.
5. **Lancer le play dormant supervisé** : après recette R0 complète, faire choisir le seuil au commanditaire et conserver la validation humaine.
6. **Décider du connecteur par la preuve** : cadrer Gmail **ou** Microsoft 365 en lecture seule seulement si au moins deux pilotes partagent l'écosystème et si le manque d'historique ou le temps de recherche manuelle franchit le gate documenté ; coder un seul écosystème après accord explicite de Fathi.
7. **Apprendre des corrections** après 30 brouillons corrigés, avec règles et préférences explicites.
8. **N'autoriser C7 qu'après les gates valeur, RGPD et exploitation** : suppression-list, fournisseur UE, budget/claim global atomique, états ambigus réconciliables, kill switch, self-test et décision explicite de Fathi.
