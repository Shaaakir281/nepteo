...
- **Démonstration isolée** : réservée à l'admin d'une organisation de test vide au premier seed, puis volontairement conservée comme vitrine peuplée, identifiable et réinitialisable. Le préflight des données réelles, les identifiants fictifs namespacés, les analyses scopées et l'enrichissement web désactivé empêchent le mélange. Le verrou distribué couvre aussi les mutations mémoire, onboarding, OAuth/connecteurs et sync dans une section critique complète ; une sauvegarde corrompue bloque tout seed. Sans fencing distribué, un verrou orphelin reste bloquant jusqu'à récupération manuelle vérifiée.
...
1. **Stabiliser le tenant vitrine** : conserver le jeu fictif nécessaire à la démonstration, afficher son statut partout, certifier ses comptages/signaux attendus et recetter un cycle idempotent `reset → reseed`. Le retrait du marqueur n'est plus une condition de progression ; Google Sheets, Notion et RLS se recettent dans des organisations séparées.
...
### 2026-07-30 — Codex — **Cap corrigé : la démo devient un tenant vitrine durable**
