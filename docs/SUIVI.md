...
> **Références de recette** : `docs/demo/GUIDE-TEST.md` pour le parcours, `docs/tests/SCORECARD-COMMANDITAIRE.md` pour mesurer la valeur, `docs/TESTS.md` pour les connecteurs et `docs/tests/SMOKE-AUTH-RLS.md` pour l'isolation Supabase. Au 2026-08-14, Azure sert à 100 % la révision `nepteo-prod--0000035`, sur l'image du merge META-METRICS `75120370746e48c15b37c74e7d19188334bf72db`. La PR #45, sa CI, son déploiement et ses sondes sont attestés ci-dessous. Le smoke RLS multi-rôles/tenants complet reste réservé à des organisations de recette séparées.

## État actuel (2026-08-14)

**CAMP-0, CAMP-1, CAMP-2, CONN-0, CONN-1, META-READ, META-METRICS, le Studio de visuels campagne-first, le scénario en un clic et le durcissement sécurité sont fusionnés dans `main` et servis par la révision courante `nepteo-prod--0000035`.** META-METRICS lit uniquement via `ads_read` et applique ses photographies atomiques dans le schéma 29 ; aucune mutation Ads n'est ouverte. Le parcours temporaire de demande d'accès testeur est implémenté localement derrière la migration `0030`, non appliquée et non publiée. La [roadmap Campagnes](projets/roadmap-campagnes-supervisees.md) conserve G1 avant BUDGET-RESULTS.
...
déployé et le projet Supabase lié est attesté au schéma `29` avec ses trois
tables. OAuth `ads_read`, le compte `Fathi Mts`, EUR, Europe/Paris et une lecture
...
- **Schéma Campagnes + Créatif + META-METRICS — Supabase à 29, code META-METRICS déployé** : les migrations `0022` à `0029` sont présentes dans le projet lié, qui expose `app_schema_version = 29`, `ad_metric_sync_runs`, `ad_campaigns` et `ad_metric_results`. Le smoke META-METRICS atomique/inter-tenant reste distinct. La migration locale `0030` du parcours pilote n'est pas appliquée.
- **Connecteurs Growth — META-METRICS déployé, parcours pilote local** : CONN-0 conserve le catalogue existant et rend les capacités honnêtes ; CONN-1 porte consentement, pause, reprise et révocation. Le callback Meta réel, la liste de comptes et une lecture vide sont recettés. Le parcours local `requested → ready → connected` ne contacte pas Meta et laisse l'ajout du testeur et sa notification à l'exploitation.
...
- **Readiness de schéma active** : `0016` introduit le marqueur privé. Le projet Supabase lié est vérifié à 29 après `0029` et la production exige 29. Le code pilote local et le workflow modifié exigent 30 afin d'empêcher son déploiement avant recette et application explicites de `0030`.
- **Release applicative courante attestée** : la PR #45 est fusionnée au merge `75120370746e48c15b37c74e7d19188334bf72db`. Le workflow protégé `31748499800` est vert et la même image est servie par `nepteo-prod--0000035`, active et prête, à 100 % du trafic.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-08-14, la base est à la version 29 et la révision déployée est `nepteo-prod--0000035`, sur l'image `nepteoacr27de3b.azurecr.io/nepteo:75120370746e48c15b37c74e7d19188334bf72db`. Les contrôles `/api/health` et `/api/ready` répondent 200 sur le domaine public et le FQDN Azure.
...
### 2026-08-14 — Codex — **Parcours pilote Meta implémenté localement après déploiement de META-METRICS**
- La PR #45 a été fusionnée puis déployée dans `nepteo-prod--0000035`; l'image immuable du merge `75120370746e48c15b37c74e7d19188334bf72db`, le schéma 29 et les quatre sondes publiques sont verts.
- Le parcours temporaire Meta est implémenté localement derrière `0030` : demande d'accès par organisation/utilisateur, e-mail Facebook normalisé, lien de profil facultatif, états `requested → ready → connected`, lecture RLS du seul demandeur et RPC service-role pour les transitions.
...
- Aucun mot de passe, jeton, App ID ou secret n'est demandé ; aucun appel Meta, envoi, migration distante, commit, push, PR ou déploiement n'a été effectué pour ce lot.
- `0030` doit être recettée puis appliquée avant publication : le code et le workflow locaux exigent volontairement le schéma 30. G1 META-METRICS reste ouvert pour le smoke atomique/JWT et l'échantillon non vide.
