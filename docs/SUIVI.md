...
> **Références de recette** : `docs/demo/GUIDE-TEST.md` pour le parcours, `docs/tests/SCORECARD-COMMANDITAIRE.md` pour mesurer la valeur, `docs/TESTS.md` pour les connecteurs et `docs/tests/SMOKE-AUTH-RLS.md` pour l'isolation Supabase. Au 2026-08-03, Azure sert à 100 % la révision `nepteo-prod--0000015`, image du merge `82d9618f123acd177b116a73399c0f899a3f6118`, avec health/readiness HTTP 200 après déploiement. Cette révision corrige le délai de recherche de l’identité mais ne contient pas encore la prise en main guidée. Le smoke RLS multi-rôles/tenants se valide dans des organisations de recette séparées.

## État actuel (2026-08-09)

**CAMP-0, CAMP-1, CAMP-2, CONN-0, CONN-1 et META-READ sont fusionnés dans `main` ; leur disponibilité en ligne reste à attester séparément.** L'agent lit, détecte et propose pour les seules capacités autorisées. Les lots Campagnes et Connecteurs ne publient aucun visuel, n'envoient rien et n'autorisent aucune écriture Ads. La [roadmap Campagnes supervisées et intégrateurs](projets/roadmap-campagnes-supervisees.md) séquence la suite ; elle ne remplace ni la [roadmap valeur — tests commanditaires](projets/roadmap-valeur-commanditaires.md), ni la [roadmap de prise en main](projets/roadmap-prise-en-main.md), qui gardent leurs propres gates.

**Mise à jour du 9 août — états séparés.** Le projet Supabase lié est vérifié à `app_schema_version = 27` depuis le 2026-08-08. Les lots CONN-0, CONN-1 et META-READ sont fusionnés dans `main` et n'ajoutent aucune migration après `0027`. La refonte visuelle allégée et le studio créatif campagne-first sont implémentés dans la branche de travail courante, mais ne sont ni fusionnés, ni déployés. Leur migration `0028_creative_assets.sql` exige le schéma 27 et porte la readiness applicative à 28. Une migration, une fusion Git et un déploiement restent trois états distincts.
...
- **CAMP-0 — fusionné dans `main`, déploiement et recette à attester** : « Nouvelle campagne » part désormais de onze champs explicites sans valeur engageante par défaut. Le serveur nettoie et valide le brief, recalcule le plan et le budget total, borne deux variantes, puis `0025` crée atomiquement et de façon idempotente une action `launch_campaign` `proposed` avec son journal. La validation est libellée « Validée — non lancée » ; aucun bouton Exécuter, aucune outbox et aucun appel Ads/email ne sont ajoutés.
- **CAMP-1 — fusionné dans `main`, déploiement et recette à attester** : le studio v2 rend éditables un à cinq adsets, leur allocation totale, deux à six hooks et leur sélection ; les formats sont dérivés du canal côté serveur. La preuve agrège 30 jours du fournisseur correspondant et ne rend une projection qu'avec au moins 7 jours distincts, une dépense positive et 10 conversions ; sinon aucun benchmark ne remplace les données manquantes. Une veille concurrentielle séparée exige confirmation et passe par `runResearch`. `0026` distingue intention et snapshot serveur pour conserver l'idempotence malgré l'évolution des métriques. La sortie reste une action `launch_campaign` `proposed` avec son journal, sans exécution, outbox, publication ni mutation Ads/email.
- **CAMP-2 — fusionné dans `main`, déploiement et recette à attester** : le cockpit de décision agrège `ad_metrics` sur deux fenêtres calendaires explicites et rattache l'historique réel de `actions`/`journal`. Dépense, conversions, revenu, CAC, ROAS, CPM et CTR, événements et recommandations conservent une source inspectable ; une confiance non calibrée reste absente. Le résumé opérationnel distingue contrôles persistés, dernier démarrage et connecteurs hors scénario ; la recherche prospect est expurgée et bornée. Un rapport strict 7 jours contre 7 jours et quatre questions fixes sont dérivés sans IA. La récence des métriques produit seulement « Données récentes » ou « Historique » : aucun état « Active »/« Terminée » n'est inféré sans statut fournisseur. Le refus d'une proposition exige un motif. `0027` rend atomique et idempotente la création des actions `ads_pause_*` avec leur journal, adopte les anciennes propositions à confiance fixe avec une trace liée, refuse les doublons ouverts préexistants sans les fusionner et ferme claim comme finalisation à toute pause Ads ; une validation affiche « Validée — non appliquée ». Aucun appel Ads, outbox, IA, envoi ou dépense n'est ajouté.
- **Schéma Campagnes — Supabase à 27** : les migrations `0022` à `0027` sont présentes dans `main` et le projet Supabase lié expose désormais `app_schema_version = 27`. Cela ne prouve pas à lui seul que la dernière application Campagnes ou Connecteurs est déployée sur Azure ; la révision applicative et sa recette doivent être attestées séparément avant toute conclusion de disponibilité produit.
- **Connecteurs Growth — fusionnés dans `main`, sans migration supplémentaire** : CONN-0 conserve le catalogue existant et rend les capacités honnêtes ; CONN-1 porte consentement, pause, reprise et révocation ; META-READ ajoute une lecture Meta bornée. Ces lots réutilisent `connectors`, `journal`, `ad_metrics` et `connectors.config`, sans toucher aux tables ou RPC créatives. Leur déploiement et leur recette en ligne restent à distinguer de leur fusion Git.
- **Créatif IA campagne-first — branche locale seulement** : `/contenu` part d'une campagne récente, reprend son message et recommande la Story 9:16 pour Meta ; la création libre reste secondaire et paginée. La génération explicite utilise `gpt-image-2`, puis persiste le JPEG dans un bucket privé avec versions et sélection. Une version retenue est validée atomiquement avec la campagne ; si la campagne a été approuvée sans visuel, un choix explicite peut encore finaliser son visuel ensuite. L'appel OpenAI ne garde pas le verrou organisation ; un chemin pending et le cron réconcilient les uploads interrompus par un claim SQL sous verrou avant toute suppression Storage. Les quotas sont réservés avant l'appel payant : vingt tentatives par organisation/jour protègent le coût et cinq réservations actives ou réussies bornent les versions d'une campagne ; un échec libère la place de campagne sans effacer son coût quotidien. Aucun visuel n'est publié chez un fournisseur, et ces assets n'alimentent pas encore `ad_metrics` ni l'audit créatif Meta.
...
- **Prise en main guidée — fusionnée dans `main`, déploiement et recette à attester** : l’onboarding propose désormais les voies exemple/entreprise réelle avant le formulaire ; `/prise-en-main` porte onze missions et ouvre les vrais écrans ; la progression locale versionnée ne conserve ni URL ni contenu métier. Le scénario choisi n’est jamais chargé automatiquement. La fusion Git est acquise ; le déploiement et la recette en ligne restent à attester.
...
- **Deux voies de test techniques exclusives déployées ; RPC CSV et chargements/analyses recettés** : **A**, l'un des trois scénarios Nepteo V2 certifiés dans une organisation vide et classé `certified-demo` ; **B**, un environnement de test alimenté par des données saisies ou importées par le testeur via interface, connecteur ou CSV, qu'elles soient réelles ou synthétiques. Le scénario doit être retiré avant toute saisie ou tout import. Le préflight, les marqueurs namespacés et le verrou distribué empêchent le mélange ; une sauvegarde corrompue bloque tout seed. Sans fencing distribué, un verrou orphelin reste bloquant jusqu'à récupération manuelle vérifiée. Le smoke réel de `0021` est vert sur `E2E_RLS_CSV_OWN` / `E2E_RLS_CSV_OTHER`. Les cycles de chargement et d'analyse des trois variantes sont verts ; chaque scénario a produit six propositions avec une console vide et Atelier Northwind reste actif. Le gate `reset → reseed → préparation → exécution` reste ouvert.
...
- **Readiness de schéma active** : `0016` introduit le marqueur privé. Le projet Supabase lié est vérifié à 27 après `0022` à `0027`. Le lot créatif local ajoute `0028` et exige 28 dans l'application et le workflow ; il ne doit être appliqué qu'après intégration du dernier `main`, tests complets et contrôle qu'aucune nouvelle migration parallèle n'a pris ce numéro.
- **Dernière release applicative historiquement attestée dans ce journal** : la PR #17 et la révision Azure associée restent la dernière recette complète détaillée ci-dessous. Depuis, Supabase a été porté séparément au schéma 27 ; ne plus présenter automatiquement base et application comme alignées sans vérifier le SHA et la révision Azure actifs.
- **Qualité de release et recette vitrine** : la CI de PR `30620564365` est verte sur le head `28781aad52564f02fcee1c0dda4b5ee5291836b8` ; la CI `main` `30620691704` et le déploiement `30620812901` sont verts, comme les six contrôles HTTP 200. Les 385/385 tests, le typecheck, le lint et le build de 24 routes sont verts ; la revue croisée ne relève aucun P1/P2. La recette authentifiée confirme le nouveau vocabulaire, la source du briefing « à partir des données d'exemple du scénario Nepteo », l'identité en lecture seule et la redirection de `/onboarding/identite` vers « Mon entreprise » avec le message de garde ; Northwind reste actif sans mutation et la console est vide. Le gate `reset → reseed → préparation → exécution` reste ouvert.
- **Contrat PR #17 déployé** : la surface emploie « scénario d'exemple Nepteo » et « données d'exemple » pour la voie A, puis « environnement de test » pour les données du testeur réelles ou synthétiques. Le préflight serveur refuse `unsafe_existing_data` avant toute mutation ; si le rendu client est devenu obsolète, cette réponse pose un latch fail-closed qui désactive immédiatement les trois chargements jusqu'au rafraîchissement.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-07-31, la base est à la version 21 et la révision déployée est `nepteo-prod--0000011`, issue du SHA `704efabd80de434ea2619cd993ae87427c114838`, image `nepteoacr27de3b.azurecr.io/nepteo:704efabd80de434ea2619cd993ae87427c114838`, digest `sha256:fe6cafbe991c45952262e33be965e4ba09239ff421a86dce80231117a3504425`. Elle est latest et ready, Healthy/Provisioned/RunningAtMaxScale avec une réplique, sert 100 % du trafic et les six contrôles HTTP sont verts.

## Prochaines étapes (dans l'ordre)

Pour l'intégration actuelle, les lots Connecteurs accessibles sont déjà dans `main` et ne réservent aucune migration après `0027`. Le lot créatif vient ensuite en `0028`. Cette numérotation doit néanmoins être recontrôlée juste avant fusion si un nouveau chantier parallèle ajoute une migration.

### Intégration du lot créatif et recette croisée
...
3. **Terminé en local — vérifier la séquence exacte `0022 … 0028`** : la suite complète, typecheck, lint et build passent. Les contrôles SQL automatisés restent statiques et ne remplacent pas la recette Supabase réelle ci-dessous.
4. **Appliquer `0028` d'abord sur une base Supabase de staging/recette distincte déjà à 27**, vérifier version 28, RLS/JWT, concurrence, bucket privé, tables, RPC, pending/cron et privilèges. La production ne doit jamais être sa première exécution et exige une autorisation explicite.
5. **Recetter conjointement** pause/reprise/révocation et lecture Meta, puis campagne → Story → nouvelle version → sélection → validation campagne + visuel. Aucun appel d'écriture fournisseur n'est autorisé par cette recette.
...
- **Migrations parallèles** : le numéro d'une migration de branche n'est jamais une réservation durable. Recontrôler le dernier numéro de `origin/main` juste avant l'intégration et renuméroter le lot qui fusionne ensuite ; conserver les prérequis et la readiness correspondants.
...
**Compatibilité Connecteurs vérifiée** : CONN-0, CONN-1 et META-READ sont déjà fusionnés dans `main`, n'ajoutent aucune migration après `0027` et ne touchent ni `creative_assets` ni les RPC créatives. Le seul chevauchement directement additif est `lib/journal.ts`, où tous les libellés doivent être conservés. Le projet Supabase lié est vérifié à la version 27 ; `0028` reste donc le bon numéro à cet instant.

**Vérifications locales** : le dernier `origin/main` est intégré ; `npm run typecheck`, le lint complet, `git diff --check`, les 567/567 tests et `npm run build` sont verts, avec 29 pages statiques générées et toutes les routes dynamiques construites. La recette visuelle du workspace réel est verte en desktop et en mobile 390 × 844, sans débordement détecté, avec changement de campagne et passage en création libre. Les contrôles SQL automatisés de `0028` restent statiques : ils ne prouvent ni l'exécution PostgreSQL, ni les RLS JWT, ni les courses concurrentes, ni le nettoyage Storage réel. Aucun push, PR, migration distante `0028`, déploiement, publication Ads ou envoi externe n'a été effectué.

**Reste** : pousser et faire relire le lot, appliquer `0028` d'abord sur une base de staging à 27, y exécuter les smokes PostgreSQL/RLS/concurrence/Storage, puis seulement porter la base de production à 28, déployer l'application et jouer la recette croisée Story + Connecteurs.
