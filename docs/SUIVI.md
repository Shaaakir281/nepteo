...
- **Release applicative courante attestée** : la PR #29 a passé la CI `31332578671`, puis a été fusionnée au merge `c5e7148ad62908a52536f6b2b52fd32ed0c357c0`. Le déploiement `31332676182` est vert sur la révision Azure `nepteo-prod--0000022`, qui sert 100 % du trafic.
- **Qualité de release et recette Story** : 571/571 tests, typecheck, lint et build de 29 routes sont verts localement et dans la CI. Sur le domaine public, `/` redirige directement en `307` vers `/login`, dont la réponse finale est 200 ; `/api/health` et `/api/ready` répondent directement 200. Health/readiness sont également verts sur le FQDN Azure. Le contrôle responsive desktop/mobile confirme l'absence de débordement ; le parcours authentifié campagne-first → génération `gpt-image-2` → sélection → validation atomique → persistance privée a été exécuté une seule fois. Le gate `reset → reseed → préparation → exécution` reste ouvert.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-08-09, la base est à la version 28 et la révision déployée est `nepteo-prod--0000022`, issue du SHA `c5e7148ad62908a52536f6b2b52fd32ed0c357c0`, image `nepteoacr27de3b.azurecr.io/nepteo:c5e7148ad62908a52536f6b2b52fd32ed0c357c0`. Elle est latest et ready, `Succeeded`/`Running`, sert 100 % du trafic et les contrôles HTTP sont verts.
