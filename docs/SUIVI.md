...
> **Références de recette** : `docs/demo/GUIDE-TEST.md` pour le parcours, `docs/tests/SCORECARD-COMMANDITAIRE.md` pour mesurer la valeur, `docs/TESTS.md` pour les connecteurs et `docs/tests/SMOKE-AUTH-RLS.md` pour l'isolation Supabase. Au 2026-07-31, les six contrôles de `/`, `/api/health` et `/api/ready` sur les deux origines sont verts sur la révision courante `nepteo-prod--0000011`. La PR #17 livre une seule voie à la fois : un scénario d'exemple Nepteo (`certified-demo` en interne) et ses « données d'exemple », ou un « environnement de test » alimenté par des données saisies ou importées par le testeur, réelles ou synthétiques. Le smoke RLS multi-rôles/tenants se valide dans des organisations de recette séparées.
