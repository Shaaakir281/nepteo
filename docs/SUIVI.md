...
- **Release applicative courante attestée** : la PR #46 est fusionnée au merge `a7cd9c2e070c866efc8acd6645451d168549f48d`. La CI `main` `31783054120` et le workflow protégé `31783151837` sont verts ; la même image est servie par `nepteo-prod--0000036`, active, saine et prête, à 100 % du trafic.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-08-14, la base est à la version 30 et la révision déployée est `nepteo-prod--0000036`, sur l'image `nepteoacr27de3b.azurecr.io/nepteo:a7cd9c2e070c866efc8acd6645451d168549f48d`. Les contrôles `/api/health` et `/api/ready` répondent 200 sur le domaine public et le FQDN Azure.
