...
**Mise à jour du 9 août — états séparés.** Le projet Supabase lié `hrqnzorapjnosjphftur` est vérifié à `app_schema_version = 28`. Les deux tables créatives, le bucket privé et les cinq RPC attendues de `0028` sont visibles en lecture distante. Les lots CONN-0, CONN-1 et META-READ sont fusionnés dans `main`. La refonte visuelle allégée et le studio créatif campagne-first restent dans la branche locale : ils ne sont ni poussés, ni fusionnés, ni déployés. Une migration, une fusion Git, un déploiement et une recette runtime restent quatre états distincts.
...
- **Schéma Campagnes + Créatif — Supabase à 28** : les migrations `0022` à `0028` sont présentes dans le projet Supabase lié, qui expose `app_schema_version = 28`. Cela ne prouve pas que la dernière application Campagnes, Connecteurs ou Créatif est déployée sur Azure ; la révision applicative et sa recette doivent être attestées séparément avant toute conclusion de disponibilité produit.
...
4. **Schéma distant vérifié — `0028` est présent sur le projet lié**, avec version 28, bucket privé, tables et cinq RPC. Il reste à attester la provenance staging/recette et à jouer réellement RLS/JWT, concurrence, pending/cron et Storage avant de considérer la porte base entièrement verte.
