...
**La correction du timeout d’identité est déployée ; la prise en main guidée est en intégration locale, non publiée.** L'agent lit, détecte, propose et prépare des messages dans l'outbox. Le lanceur R2, lui, crée uniquement une proposition : il n'écrit pas dans l'outbox et n'envoie **rien à l'extérieur**. La [roadmap valeur — tests commanditaires](projets/roadmap-valeur-commanditaires.md) et la [roadmap de prise en main](projets/roadmap-prise-en-main.md) gardent des gates distincts.
