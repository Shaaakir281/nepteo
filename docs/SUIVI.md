...
1. **Fathi (manuel)** : ~~connecter Notion~~ **fait**. `npm test` **17/17 (Node 22)**. Reste : `npm run build` (Windows) pour le check final, et dérouler le parcours §3 dans l'app (3 propositions + badges de priorité + dédup). **Backlog acté** : écran de correspondance de colonnes configurable (cf. DECISIONS) pour les bases clientes aux intitulés exotiques.
