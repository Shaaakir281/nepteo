...
- **Release applicative courante attestée** : la PR #33 a passé la CI `31372881341`, puis a été fusionnée au merge `6f54cee5cc7c594956515cea2a94204d5da2a5a2`. Le déploiement `31373032666` est vert sur la révision Azure `nepteo-prod--0000024`, latest et ready, qui sert 100 % du trafic.
- **Qualité de release et recettes ciblées** : 575/575 tests, typecheck, lint et build de 29 routes sont verts. `/api/health` et `/api/ready` répondent 200. Sans paramètre, avec `token_hash` et `type=email`, avec `token_hash` et `type=signup`, ou avec un `code` invalide, `/auth/confirm` répond 307 vers `https://nepteo.bogasolution.com/login?...`, jamais vers `0.0.0.0`. En production, Afficher/Masquer conserve le mot de passe saisi sur connexion et inscription. La recette Story authentifiée de la PR #29 reste valide ; le clic positif d'un email neuf et le clic Déconnexion puis Retour depuis une session mobile authentifiée restent ouverts. Le gate `reset → reseed → préparation → exécution` reste ouvert.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-08-10, la base est à la version 28 et la révision déployée est `nepteo-prod--0000024`, issue du SHA `6f54cee5cc7c594956515cea2a94204d5da2a5a2`, image `nepteoacr27de3b.azurecr.io/nepteo:6f54cee5cc7c594956515cea2a94204d5da2a5a2`. Elle est latest et ready, `Succeeded`/`Running`, sert 100 % du trafic et les contrôles HTTP sont verts.
