...
- **Base et application alignées en production** : les migrations `0012` à `0020` ont été appliquées manuellement le 2026-07-30 et `app_schema_version = 20` est enregistré depuis `2026-07-30T06:02:14Z`. Azure sert l'image `nepteoacr27de3b.azurecr.io/nepteo:73f7e79ea64ed72102a1f59f6344add8122dcdc9`, révision `nepteo-prod--0000003`, avec 100 % du trafic.
- **Qualité de release** : la CI de la PR #5 et celle de `main` ont validé tests, lint, typecheck et build. Le workflow de production a validé le préflight schéma 20, la construction de l'image, la mise à jour Azure, la liveness et la readiness. Le smoke public navigateur ne remonte aucune erreur console.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-07-30, la base est à la version 20 et Azure sert la révision `nepteo-prod--0000003`, image `73f7e79`, avec 100 % du trafic. `/`, `/api/health` et `/api/ready` répondent HTTP 200.

## Prochaines étapes (dans l'ordre)

1. **Livrer puis recetter le lot local du smoke authentifié** : reconnaître et retirer le connecteur démo orphelin hérité de l'ancienne version, puis vérifier la cohorte prospects partagée, ses chiffres dédoublonnés et ses suspensions fail-closed. Ne supprimer aucune donnée réelle lors de cette opération.
...
- **Doublons multi-source et opposition** : distinguer la déduplication visuelle de la canonicalisation métier. Le lot local impose « terminal/opposé gagne », neutralise les statuts actifs contradictoires et conserve le dernier contact valide avant toute intersection de snapshot. Tant que ce contrat n'est pas livré puis recetté sur deux sources réelles, ne pas promouvoir le play dormant.
