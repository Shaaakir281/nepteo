...
> **Références de recette** : `docs/demo/GUIDE-TEST.md` pour le parcours, `docs/tests/SCORECARD-COMMANDITAIRE.md` pour mesurer la valeur, `docs/TESTS.md` pour les connecteurs et `docs/tests/SMOKE-AUTH-RLS.md` pour l'isolation Supabase. Au 2026-07-30, `/`, `/api/health` et `/api/ready` répondent HTTP 200 sur la révision Azure `nepteo-prod--0000007`. L'organisation présentée aux commanditaires est un environnement de test qui suit une seule voie à la fois : scénario Nepteo V2 certifié ou données autorisées du testeur. Le smoke RLS multi-rôles/tenants se valide dans des organisations de recette séparées.
...
- **Deux voies de test exclusives** : **A**, l'un des trois scénarios Nepteo V2 certifiés dans une organisation vide, identifiable, réinitialisable et seul autorisé à porter le libellé « données fictives » ; **B**, des données autorisées apportées par le testeur via connecteur ou CSV, présentées comme « environnement de test ». Le scénario doit être retiré avant tout import. Le préflight, les marqueurs namespacés et le verrou distribué empêchent le mélange ; une sauvegarde corrompue bloque tout seed. Sans fencing distribué, un verrou orphelin reste bloquant jusqu'à récupération manuelle vérifiée.
...
- **Base migrée en avance de façon additive** : `app_schema_version = 21` est enregistré en production depuis `2026-07-30T17:40:26Z`. Azure sert encore l'application précédente, compatible avec toute version `>= 20`, image `nepteoacr27de3b.azurecr.io/nepteo:a2bbc34dcb97ab00951a3efa631c4f7c0a0428ca`, révision `nepteo-prod--0000007`, avec 100 % du trafic.
- **Qualité de release** : les CI de la PR #11 et de `main` ont validé tests, lint, typecheck et build. Le workflow `30550603760` a validé le préflight schéma 20, construit l'image immuable, mis à jour Azure puis contrôlé liveness et readiness. La recette administrateur authentifiée confirme le bandeau vitrine et les aperçus connecteurs sans erreur console.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-07-30, la base est à la version 21 et Azure sert encore la révision `nepteo-prod--0000007`, image `a2bbc34`, avec 100 % du trafic. `/`, `/api/health` et `/api/ready` répondent HTTP 200.

## Prochaines étapes (dans l'ordre)

1. **Recetter `0021` et les deux voies sans mélange** : jouer les RPC CSV sur une organisation de test vide, puis certifier les trois variantes du scénario Nepteo V2 avec leurs comptages/signaux et le cycle idempotent `reset → reseed`. Vérifier qu'il faut retirer ce scénario avant d'importer les données autorisées d'un testeur. Le CSV V1 doit rester UTF-8, borné à 900 Ko/5 000 lignes, mappé sans ambiguïté, minimisé, atomique, verrouillé et journalisé.
2. **Recetter le conflit de statut multi-source avant pilote** : vérifier sur deux connecteurs que, pour un même contact, tout statut terminal/opposé bloque la relance et que deux statuts actifs contradictoires suspendent la cible, de la proposition jusqu'à la préparation.
3. **Terminer le smoke RLS dédié** : compte neuf, rôles, lecture/écriture autorisées et refus inter-tenant sur des organisations `E2E_RLS_*` ; contrôler à nouveau l'absence d'envoi externe.
4. **Recetter C8, R1A, R1B et R2** : callbacks Google/Notion, isolation démo, dates Sheets/Notion, Top 5, seuils 30/45, sélection oldest-first, exclusion des vagues antérieures, verdicts/retouches, cohorte atomique, conservation de `action_kind` et scorecard locale non-démo/fail-closed.
5. **Promouvoir sur un pilote dédié** : noter l'image et la révision effectivement recettées dans la scorecard.
6. **Lancer le play dormant supervisé** : après recette R0 complète, faire choisir le seuil au commanditaire et conserver la validation humaine ; réunir 3 testeurs/30 recommandations pour le checkpoint qualitatif, puis 50 recommandations/deux semaines pour la décision produit, avec consolidation programme pseudonymisée hors application et sans agrégation inter-tenant.
...
**Validation et livraison en cours** : **359/359 tests**, typecheck, lint, `git diff --check` et build Next.js de production (**24 pages/routes**) sont verts. Les revues indépendantes démo et CSV ne relèvent plus de P1/P2. Le commit `b1f760d` est poussé sur `codex/demo-scenarios-v2` et `0021` est appliquée en production (`app_schema_version = 21` à `2026-07-30T17:40:26Z`). Le 2026-07-31, le smoke RPC réel a validé sur `E2E_RLS_CSV_OWN` / `E2E_RLS_CSV_OTHER` le refus inter-tenant, l'import, le rollback tardif, le rejeu idempotent et le retrait. Restent la fusion de la PR, le déploiement applicatif et la recette navigateur.
