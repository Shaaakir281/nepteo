...
**La correction du timeout d’identité est déployée ; CAMP-0, CAMP-1 et CAMP-2 sont implémentés localement et non publiés.** L'agent lit, détecte, propose et prépare des messages dans l'outbox pour les seules actions exécutables existantes. Les lots Campagnes locaux créent uniquement des propositions : ils n'écrivent pas dans l'outbox et n'envoient **rien à l'extérieur**. La [roadmap Campagnes supervisées et intégrateurs](projets/roadmap-campagnes-supervisees.md) séquence désormais CAMP-3 à CAMP-10 et les connecteurs de la maquette en micro-lots ; elle ne remplace ni la [roadmap valeur — tests commanditaires](projets/roadmap-valeur-commanditaires.md), ni la [roadmap de prise en main](projets/roadmap-prise-en-main.md), qui gardent leurs propres gates.

État du produit déployé, avec le niveau de recette précisé dans chaque lot :
...
- **Prise en main guidée — branche locale seulement** : l’onboarding propose désormais les voies exemple/entreprise réelle avant le formulaire ; `/prise-en-main` porte onze missions et ouvre les vrais écrans ; la progression locale versionnée ne conserve ni URL ni contenu métier. Le scénario choisi n’est jamais chargé automatiquement. Aucun push, fusion ou déploiement de ce lot n’a encore eu lieu.
...
- **Readiness de schéma active** : `0016` introduit le marqueur privé après avoir prouvé les invariants critiques de `0012` à `0015`. Les migrations additives suivantes portent les contrats terrain, CSV et recherche jusqu'à `0024`. Les lots Campagnes locaux ajoutent `0025`, `0026` puis `0027` et exigent désormais 27 ; ces migrations ne sont appliquées nulle part par la présente session. `/api/ready` échoue fermée si Supabase ou la version requise sont indisponibles.
- **Base et application alignées en production** : `app_schema_version = 21`. La PR [#17](https://github.com/Shaaakir281/nepteo/pull/17), fusionnée dans `main` au SHA `704efabd80de434ea2619cd993ae87427c114838`, est déployée sur `nepteo-prod--0000011` avec l'image `nepteoacr27de3b.azurecr.io/nepteo:704efabd80de434ea2619cd993ae87427c114838`, digest `sha256:fe6cafbe991c45952262e33be965e4ba09239ff421a86dce80231117a3504425`. La révision est latest et ready, Healthy/Provisioned/RunningAtMaxScale avec une réplique, et sert 100 % du trafic.
...
Pour la branche Campagnes, l'ordre est `REL-0 → recette en ligne CAMP-0 → CAMP-1 → CAMP-2 → CONN-0`, chaque publication exigeant une autorisation distincte. Cet ordre documente les releases ; il ne permet pas de contourner les gates valeur et sécurité ci-dessous, et aucun lot fournisseur ne démarre sur la seule foi de la maquette.
