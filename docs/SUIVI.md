...
**CAMP-0, CAMP-1, CAMP-2, CONN-0, CONN-1, META-READ, META-METRICS, le Studio de visuels campagne-first, le scénario en un clic et le durcissement sécurité sont fusionnés dans `main` et servis par la révision courante `nepteo-prod--0000035`.** META-METRICS lit uniquement via `ads_read` et applique ses photographies atomiques dans le schéma 30 ; aucune mutation Ads n'est ouverte. La migration `0030` du parcours temporaire de demande d'accès testeur est appliquée en production ; son code est porté par la PR #46 et ne devient servi qu'après fusion et promotion Azure. La [roadmap Campagnes](projets/roadmap-campagnes-supervisees.md) conserve G1 avant BUDGET-RESULTS.
...
- **Readiness de schéma active** : `0016` introduit le marqueur privé. Le projet Supabase lié est vérifié à 30 après `0030` et le workflow du parcours pilote exige 30, ce qui empêche toute promotion Azure avant application explicite de la migration.
...
Environnement : Supabase `hrqnzorapjnosjphftur`, repo GitHub `Shaaakir281/nepteo` (branche `main`), dev local port 3001, Node 22+. Production Azure dans `francecentral`, domaine `https://nepteo.bogasolution.com` et HTTPS opérationnels. Au 2026-08-14, la base est à la version 30 et la révision déployée reste `nepteo-prod--0000035` jusqu'à la promotion de la PR #46, sur l'image `nepteoacr27de3b.azurecr.io/nepteo:75120370746e48c15b37c74e7d19188334bf72db`. Les contrôles `/api/health` et `/api/ready` répondent 200 sur le domaine public et le FQDN Azure.
...
- Aucun mot de passe, jeton, App ID ou secret n'est demandé ; aucun appel Meta ni envoi n'a été effectué. Le commit `debb3f2`, la PR #46 et sa CI sont publiés.
