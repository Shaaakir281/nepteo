...
- **2026-07-26** — **C4 terminé** (détail dans `docs/SUIVI.md`). Nav à **5 entrées** ; « Plan du mois » devient le bandeau « Cap du mois » d'Aujourd'hui (`_components/plan-banner.tsx`, rendu seulement s'il y a des données, explicitement en lecture seule) ; « Connecteurs » et « Agent » deviennent des onglets de « Mon entreprise » ; `/contenu` sort du menu mais reste vivante (bouton sur `/campagnes` + CTA du cap). `/plan` redirige en permanent, `/connecteurs` et `/agent` en temporaire (amendement à valider avec Charly), `searchParams` conservés. **Aucun moteur pur touché**, tests **136 inchangés** (la liste blanche des `ctaHref` reste valide), `tsc` exit 0. ADR consigné dans `DECISIONS.md`. Prochain geste : **C5** (l'autonomie en un réglage), qui devient plus simple — tout est dans `entreprise/_components/agent-panel.tsx`.
